...
<section id="comments" class="comments-area py-5">
	...
	/* translators: %s: Asterisk symbol (*). */
		' ' . esc_attr__( 'Required fields are marked %s', 'knowledgecenter' ),
		'<span class="required has-text-danger">*</span>'
	);
	$form_args        = array(
		'class_form'           => '',
		/* translators: 1: Link to author profile 2: Author 3: Link for logout */
		'logged_in_as'         => '<p class="help has-text-right">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'knowledgecenter' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( get_permalink() ) ) . '</p>',
		'comment_field'        => '<div class="field"><div class="control"><textarea name="comment" class="textarea is-size-7" aria-required="true" placeholder="' . esc_attr__( 'Add Comment', 'knowledgecenter' ) . '">' . '</textarea></div></div>',
		...
		'comment_notes_before' => '<p class="comment-notes help">' . esc_html__( 'Your email address will not be published.', 'knowledgecenter' ) . ( $req ? $required_text : '' ) . '</p>',
		'fields'               => apply_filters( 'knowledgecenter_comment_form_default_fields', array(
			'author' => '<div class="field is-horizontal"><div class="field-label is-normal"><label class="label" for="author">' . esc_html__( 'Name', 'knowledgecenter' ) . ( $req ? ' <span class="required has-text-danger">*</span>' : '' ) . '</label></div> ' .
			...
			'email'  => '<div class="field is-horizontal"><div class="field-label is-normal"><label class="label" for="email">' . esc_html__( 'Email', 'knowledgecenter' ) . ( $req ? ' <span class="required has-text-danger">*</span>' : '' ) . '</label> </div>' .
			...
			'url'    => '<div class="field is-horizontal"><div class="field-label is-normal"><label class="label" for="url">' . esc_html__( 'Website', 'knowledgecenter' ) . '</label> </div>' .
        ...
        <div class="comments-list-area">
            <div class="comment-list">
				...
				wp_list_comments( array(
					'walker'      => new KnowledgeCenter_Comments,
					'style'       => 'div',
					'short_ping'  => true,
					'avatar_size' => 128,
				) );
				...
				<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through ?>
					...
					$args = array(
						...
						'next_text'          => esc_attr__( 'Next page', 'knowledgecenter' ),
						'add_args'           => false,
						'add_fragment'       => '',
//	'type' => 'list',
	...
	if ( ! comments_open() && '0' != get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) :
		?>
        <p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'knowledgecenter' ); ?></p>
	<?php endif; ?>
</section>
