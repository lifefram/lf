...
class Onboard {
    ...
    public function init() {
        ...
        $param = isset($_GET['wp-social-met-onboard-steps']) ? $_GET['wp-social-met-onboard-steps'] : null;
    ...
    public function finish_onboard() {
        ...
        if(class_exists('\WP_Social_Pro'))  {
            ...
            foreach ($modules as $key => $value) {
                if (isset($_POST['module_list']) && in_array($key, $_POST['module_list'])) {
        ...
        foreach (self::settings()['login'] as $key => $value) {
			if (isset($_POST['login']) && array_key_exists($key, $_POST['login'])) {
        ...
        foreach (self::settings()['share'] as $key => $value) {
            ...
            case 'show_font_from_theme':
                ...
                if (isset($_POST['share']) && array_key_exists($key, $_POST['share'])) {
    ...
    public static function settings() {
        return [
            'login'   => [
                'wp_login_page'             => [
                    'title' => 'Show button to wp-login page',
                    'value' => 'login_form',
                ],
                'email_new_registered_user' => [
                    'title' => 'Email login credentials to a newly-registered user',
                    'value' => 1,
                ],
            ],
            'share'   => [
                'show_font_from_theme'    => [
                    'title' => 'Use theme default font family',
                    'value' => 1,
                ],
