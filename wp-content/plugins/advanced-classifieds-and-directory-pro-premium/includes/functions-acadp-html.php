...
function the_acadp_listing_labels( $post_meta ) {
	...
	if ( ! empty( $badges_settings['show_new_tag'] ) ) {		
		...
		if ( $days <= (int) $badges_settings['new_listing_threshold'] ) {
			echo '<span class="label label-primary">' . $badges_settings['new_listing_label'] . '</span>&nbsp;';
		}		
	}
	...
	if ( ! empty( $badges_settings['show_popular_tag'] ) ) {	
		if ( isset( $post_meta['views'] ) && (int) $post_meta['views'][0] >= (int) $badges_settings['popular_listing_threshold'] ) {
    		echo '<span class="label label-success">' . $badges_settings['popular_listing_label'] . '</span>&nbsp;';
		}		
	}
	...
	if ( ! empty( $badges_settings['mark_as_sold'] ) ) {	
		if ( isset( $post_meta['sold'] ) && 1 == (int) $post_meta['sold'][0] ) {
    		echo '<span class="label label-danger">' . $badges_settings['sold_listing_label'] . '</span>&nbsp;';
		}		
	}
}
	...
	if ( isset( $settings['gateways'] ) ) {	
		foreach ( $gateways as $key => $label ) {			
			if ( in_array( $key, $settings['gateways'] ) ) {			
				...
				//Mel: 28/12/21. To add the class attribute so that public.js could select the radio button.
				$html .= sprintf( '<div class="radio acadp-no-margin"><label><input type="radio" name="payment_gateway" class="payment_gateway_method" value="%s"%s>%s</label></div>', $key, ( $key == end( $settings['gateways'] ) ? ' checked' : '' ), $label );
...
function the_acadp_pagination( $numpages = '', $pagerange = '', $paged = '' ) {	
     ...
     */
  	...
  	/** 
   	 ...
   	 * function. 
   	 */
	$arr_params = array( 'order', 'sort', 'view', 'lang', 'status' );
