...
function acadp_order_completed( $order ) {	
	...
	//Mel: 29/12/21. To add new post_meta data
	update_post_meta( $order['id'], 'currency_code', $order['currency_code'] );
	...
	if ( ! empty( $featured ) ) {
		...
		//Mel: 29/12/21. Add wallet address so that we can send the NFT to the wallet on admin panel
		update_post_meta( $post_id, 'user_wallet', $order['user_wallet'] );
	}
