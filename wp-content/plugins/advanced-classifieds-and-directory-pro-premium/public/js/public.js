(function( $ ) {
	...
	function acadp_sort_files() {		
		...
		$( "#acadp-upload-file-hidden" ).change( function() {			
			...
			$( '#acadp-progress-file-upload' ).addClass( 'uploading' ).html( '<div class="acadp-spinner"></div>' );
			acadp_enable_disable_image_upload();
			...
			var options2 = {
        		...
        		success: function( json, statusText, xhr, $form ) {
					...
					$.each( json, function( key, value ) {							
						if ( ! value['error'] ) {
							var html = '<tr class="acadp-file-row">' + 
								'<td class="acadp-handle"><span class="glyphicon glyphicon-th-large"></span></td>' +          	
								'<td class="acadp-file">' + 
									//'<img src="' + value['url'] + '" />' + 
									'<input type="hidden" class="acadp-file-field" name="files[]" value="' + value['id'] + '" />' +
								'</td>' + 
								'<td>' + 
									'<span class="acadp-file-url"><a href="' + value['url'] + '" />' + value['url'].split(/[\\/]/).pop() + '</a></span><br />' + 
									'<a href="javascript:;" class="acadp-delete-file" data-attachment_id="' + value['id'] + '">' + acadp.delete_label + '</a>' + 
								'</td>' +                 
							'</tr>';					
		...
		$( '.acadp-checkout-fee-field-other' ).on( 'input', function() {	
			var other_amount = 0;
		...
		$( '.payment_gateway_method' ).on( 'change', function() {	
			var total_amount = 0,
			    fee_fields   = 0;
			var retailPrice = 0;
			...
			} else if ( $( "input[name='payment_gateway']:checked" ).val() == 'ethereum' ) {
			...
			//Mel: 23/11/21
			} else if ( $( "input[name='payment_gateway']:checked" ).val() == 'eth_matic' ) {
			...
			//Mel: 26/11/21
			} else if ( $( "input[name='payment_gateway']:checked" ).val() == 'eth_avax' ) {
