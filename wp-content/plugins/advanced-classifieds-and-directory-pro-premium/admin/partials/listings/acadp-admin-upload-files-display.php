	...
	<table id="acadp-images" class="acadp-images acadp-no-border widefat">
		<tbody>
			...
			if ( isset( $post_meta['images'] ) ) {
				...
				//Mel: 06/04/22. To add a placeholder image if no image was uploaded.
...
<?php endif; ?>
	...
	document.getElementById("acadp-upload-file-ipfs").addEventListener("click", function(event) {
		...
		const dateOfBirth = document.getElementsByName("acadp_fields[1200]")[0].value;
		const dateOfDeath = document.getElementsByName('acadp_fields[1201]')[0].value;
		const spouse = document.getElementsByName('acadp_fields[1202]')[0].value;
		const siblings = document.getElementsByName('acadp_fields[1203]')[0].value;
		const children = document.getElementsByName('acadp_fields[1204]')[0].value;
		const restingPlace = document.getElementsByName('acadp_fields[1205]')[0].value;
		const occupation = document.getElementsByName('acadp_fields[1613]')[0].value;
		const city = document.getElementsByName('acadp_fields[1692]')[0].value;
		const state = document.getElementsByName('acadp_fields[1693]')[0].value;
		const imageFilename = '<?php echo esc_html( wp_basename($image_attributes[0]) ); ?>';
		
		//To create the JSON file on server before uploading to IPFS
		jQuery.ajax({
			type: "POST",
			url: window.location.origin + "/api/v1/json/create/",
			data: "fullname=" + encodeURIComponent(fullName) +
					"&bio=" + encodeURIComponent(bio) +
					"&dateofbirth=" + encodeURIComponent(dateOfBirth) +
					"&dateofdeath=" + encodeURIComponent(dateOfDeath) +
					"&spouse=" + encodeURIComponent(spouse) +
					"&siblings=" + encodeURIComponent(siblings) +
					"&children=" + encodeURIComponent(children) +
					"&restingplace=" + encodeURIComponent(restingPlace) +
			...
			success: (data) => {
				...
				//Request for the bio photo from server
				fetch('<?php echo esc_url( $image_attributes[0] ); ?>')
					...
					.then(blob => {
						...
						//Request for the bio JSON file from server
						fetch(jsonFileUrl)
							.then(res => res.blob())
							.then(blob => {
						
							const file2 = new File([blob], jsonFilename, {
							...
							jQuery.ajax({
								...
								success: (data) => {
									...
									document.getElementById("loading").innerHTML = 'Success. <a id="ipfsUri" href="https://ipfs.io/ipfs/' + data.value.cid + '"><?php esc_html_e( 'View on IPFS', 'advanced-classifieds-and-directory-pro' ); ?></a><input type="hidden" name="ipfs_cid" value="' + data.value.cid + '">';
									
									//To delete the JSON file on server after the file is uploaded to IPFS
									jQuery.ajax({
										type: "GET",
									...
									//Enable back the upload button
									document.getElementById("acadp-upload-file-ipfs").setAttribute("href", "javascript:;");
				...
				if (!empty(walletAddress)) {
					...
					button.addEventListener('click', async function (event) {
						...
						if( ipfsData != '' && ipfsData != null ) {
							...
							smartContract.methods.mintNftToken("1", "<?php echo clean_fullname( get_the_title() ); ?>", "<?php esc_html_e('Biography of a Departed', 'advanced-classifieds-and-directory-pro'); ?>", ipfsData.toString()).send({
								from: walletAddress,
								gasLimit: 4700000,
